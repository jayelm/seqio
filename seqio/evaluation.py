...
def get_valid_eval_tasks(tasks: Sequence[Task], split: str) -> Sequence[Task]:
    ...
    for task in tasks:
        if split not in task.splits:
            logging.info("Task %s has no '%s' split; skipping eval.", task.name, split)
            continue
...
PredictFnReturnType = Union[_IndicesAndPredictions, _IndicesAndPredictionsWithAuxValues]
...
def _extract_model_output(cached_model_dataset, model_fn):
    ...
    model_fn_result = model_fn(
        cached_model_dataset
    )  # pytype: disable=missing-parameter  # always-use-return-annotations
...
class Evaluator:
    ...
    def __init__(
        ...
        self._loggers = tuple(
            cls(output_dir=log_dir) for cls in logger_cls
