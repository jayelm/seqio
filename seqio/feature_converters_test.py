...
from seqio import feature_converters, test_utils
...
class FeatureConvertersTest(tf.test.TestCase):
    ...
    def test_validate_dataset_incorrect_dtype(self):
        ...
        ds = tf.data.Dataset.from_generator(
            lambda: x,
            output_types=task_feature_dtypes,
...
class PrefixLMFeatureConverter(tf.test.TestCase):
    ...
    def test_prefix_lm_packed_without_default_bos(self):
        ...
        task_feature_lengths = {"inputs": 8, "targets": 7}
        converter = feature_converters.PrefixLMFeatureConverter(pack=True, bos_id=10)
        converted_ds = converter(ds, task_feature_lengths)
...
class EncoderFeatureConverterTest(FeatureConvertersTest):
    ...
    def test_encoder_packed(self):
        x = [
            {"inputs": [8, 9, 9, 3, 4, 1], "targets": [8, 7, 4, 3, 4, 1]},
    ...
    def test_encoder_pack_long_sequences(self):
        x = [
            {"inputs": [8, 9, 9, 3, 1], "targets": [8, 7, 4, 3, 1]},
            {"inputs": [8, 3, 9, 1], "targets": [8, 3, 6, 1]},
        ]
...
class PrePackedLMFeatureConverterTest(tf.test.TestCase):
    def test_encoder_decoder_packed(self):
        x = [
            {
                "decoder_input_tokens": [0, 1, 2, 3, 4, 5, 6],
                "decoder_target_tokens": [1, 2, 3, 4, 5, 6, 7],
        ...
        converter = feature_converters.PrePackedLMFeatureConverter(pack=False)
        converted_ds = converter(ds, task_feature_lengths)
        expected = {
            "decoder_input_tokens": [0, 1, 2, 3, 4, 5, 6],
            "decoder_target_tokens": [1, 2, 3, 4, 5, 6, 7],
