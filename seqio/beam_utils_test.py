...
class BeamUtilsTest(seqio.test_utils.FakeTaskTest):
    def test_preprocess_task(self):
        def _np_to_list(ex):
            return {
                k: v.tolist() if isinstance(v, np.ndarray) else v for k, v in ex.items()
            }
    ...
    def test_write_example_tf_record(self):
        ...
        with TestPipeline() as p:
            _ = (
                p
                | beam.Create([example])
                | beam_utils.WriteExampleTfRecord(output_path=output_path, num_shards=1)
            )
    ...
    def test_write_json(self):
        ...
        with TestPipeline() as p:
            _ = p | beam.Create([data]) | beam_utils.WriteJson(output_path=output_path)
    ...
    def test_count_characters_str_dataset_in_get_stats(self):
        ...
        with TestPipeline() as p:
            ...
            util.assert_that(pcoll, util.equal_to([{"text_chars": 64, "examples": 2}]))
