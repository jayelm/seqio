...
from seqio import dataset_providers, evaluation
from seqio import metrics as metrics_lib
from seqio import preprocessors, test_utils, utils, vocabularies
...
def _sum_scores_metric_with_intermediates(targets, scores):
    ...
    weights = [sum(ord(c) for c in t) for t in targets]
    return {
        "total_score_with_intermediates": (np.array(scores) * np.array(weights)).sum()
    }
...
def get_mocked_task(
    ...
    task.metric_fns = (
        list(predict_metric_fns)
        + list(score_metric_fns)
    ...
    task.output_features = {target_field_name: dataset_providers.Feature(mock_vocab)}
    return task
...
class EvaluationTest(tf.test.TestCase):
    ...
    def test_get_valid_eval_tasks(self):
        task_no_metrics = mock.Mock(
            splits=("train", "validation"), metric_fns=[], metric_objs=[]
        )
        task_no_split = mock.Mock(splits="train", metric_fns=[lambda x: x])
        valid_task = mock.Mock(splits=("train", "validation"), metric_fns=[lambda x: x])
    ...
    def test_cache_and_measure_examples(self):
        ...
        test_utils.assert_dataset(
            cached_task_datasets["task1"], expected_task1_examples
        )
        test_utils.assert_dataset(
            cached_task_datasets["task2"], expected_task2_examples
        )
    ...
    def _evaluate_single_task(
        self, task, loggers=(), target_field_name="targets", has_aux_values=False
    ):
    ...
    def test_evaluate_using_aux_score(self):
        task = get_mocked_task(predict_with_aux_metric_fns=[_fake_aux_values_metric])
    ...
    def test_aux_scores_sorted_with_tokens(self):
        ...
        def mock_init(self):
            mock_ds = tf.data.Dataset.from_tensor_slices(["5", "6", "7"]).map(
                lambda x: {"targets_pretokenized": x}
            )
            self._cached_model_datasets = {task.name: mock_ds}
            self._cached_task_datasets = {task.name: mock_ds}
            self._eval_tasks = [task]
    ...
    def test_evaluate_non_string(self):
        ...
        def mock_init(self):
            # Dummy datasets
            mock_ds = tf.data.Dataset.from_tensor_slices([[5, 6], [6, 7]]).map(
                lambda x: {"targets_pretokenized": x}
            )
            self._cached_model_datasets = {task.name: mock_ds}
            self._cached_task_datasets = {task.name: mock_ds}
    ...
    def test_evaluate_single_task_with_postprocessor(self):
        ...
        def mock_init(self):
            # Dummy datasets
            mock_ds = tf.data.Dataset.from_tensor_slices(["e5", "e6", "e7"]).map(
                lambda x: {"targets_pretokenized": x}
            )
            self._cached_model_datasets = {task.name: mock_ds}
            self._cached_task_datasets = {task.name: mock_ds}
    ...
    def test_evaluate_mixture(self):
        ...
        task2 = get_mocked_task(
            name="task2", predict_metric_fns=[_accuracy_metric], score_metric_fns=[]
        )
        ...
        mock_ds1 = tf.data.Dataset.from_tensor_slices(["e5 e6", "e6"]).map(
            lambda x: {"targets_pretokenized": x}
        )

        mock_ds2 = tf.data.Dataset.from_tensor_slices(["e5", "e6", "e7"]).map(
            lambda x: {"targets_pretokenized": x}
        )
    ...
    def test_short_inputs_targets(self):
        ...
        dataset_fn = lambda split, shuffle_files, seed=None: ds
        register_dummy_task(
            task_name, dataset_fn=dataset_fn, metrics_fn=[_sequence_accuracy_metric]
        )
    ...
    def test_requires_sequence_length(self):
        ...
        with self.assertRaisesWithLiteralMatch(
            ...
            (
                ...
                " 'requires_sequence_length' has a `sequence_length` argument,"
                " making it incompatible with automatic sequence length detection."
                " Pass a valid `sequence_length` to `Evaluator` and try again."
            ),
        ):
            _ = Evaluator(
                mixture_or_task_name=task_name,
    ...
    def test_caching(self):
        ...
        x = [
            {"inputs": [7, 8], "targets": [3, 9], "targets_pretokenized": "ex 1"},
            {"inputs": [8, 4], "targets": [4], "targets_pretokenized": "ex 2"},
        ]
        dtypes = {
            "inputs": tf.int32,
            "targets": tf.int32,
        ...
        feature_converter.side_effect = lambda ds, length: utils.trim_and_pad_dataset(
            ds, {"inputs": 4, "targets": 4}
        )
        evaluator = Evaluator(
            mixture_or_task_name=task_name,
            feature_converter=feature_converter,
        ...
        self.assertDictEqual(
            evaluator.model_feature_shapes, {"inputs": (4,), "targets": (4,)}
        )
    ...
    def test_order_preservation(self):
        ...
        targets_pretokenized = tf.data.Dataset.from_tensor_slices(["e5", "e6", "e7"])
        ds = tf.data.Dataset.zip((targets, targets_pretokenized))
        ds = ds.map(lambda x, y: {"targets": x, "targets_pretokenized": y})
        ...
        with mock.patch.object(Evaluator, "__init__", new=mock_init):
            ...
            def mixing_order_predict_fn(
                ...
                del model_feature_shapes
                exs = [(tup[0], tup[1]["targets"]) for tup in tfds.as_numpy(ds)]
    ...
    def test_duplicate_metric(self):
        task = get_mocked_task(predict_metric_fns=[_accuracy_metric, _accuracy_metric])
        with self.assertRaisesWithLiteralMatch(
    ...
    def test_task_with_no_compute_metrics(self):
        ...
        all_metrics, all_output = evaluator.evaluate(
            compute_metrics=False, predict_fn=mock.Mock(), score_fn=self.uncalled_fn
        )
    ...
    def test_task_with_score_fn_with_intermediates(self):
        ...
        self.assertDictClose(
            {"total_score_with_intermediates": 66505}, results["mocked_test"]
        )
