...
def skip_none_value_dict_factory(data: Sequence[Tuple[str, Any]]) -> Dict[str, Any]:
...
class PyLoggingLogger(Logger):
    ...
    def __call__(
        ...
        for metric_name, metric_value in metrics.items():
            ...
            logging.info("%s/%s at step %d: %s", task_name, metric_name, step, strvalue)
...
class TensorBoardLogger(Logger):
    ...
    def _write_metric(
        ...
        if isinstance(value, metrics_lib.Scalar):
            ...
            raise TypeError(f"Value type not understood, got '{type(value).__name__}'.")
...
class TensorAndNumpyEncoder(json.JSONEncoder):
    ...
    def default(self, obj):
        ...
        elif np.issubdtype(type(obj), np.number) or np.issubdtype(type(obj), np.bool_):
...
class JSONLogger(Logger):
    ...
    def __call__(
        ...
        inferences_fname = os.path.join(self.output_dir, f"{task_name}-{step:06}.jsonl")
