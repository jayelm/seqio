...
from seqio import dataset_providers, feature_converters
...
class TasksTest(test_utils.FakeTaskTest):
    ...
    def test_metric_fn_signature(self):
        ...
        with self.assertRaisesWithLiteralMatch(
            ValueError,
            expected_error_message_prefix + "('targets', 'predictions', 'extra_param')",
        ):
            valid_task = add_task("extra_arg_metric", metric_fns=[extra_arg_metric_fn])
        ...
        with self.assertRaisesWithLiteralMatch(
            ...
            valid_task = add_task("bad_order_metric", metric_fns=[bad_order_metric_fn])
        ...
        valid_task_2 = add_task("valid_metrics_2", metric_fns=[ok_default_metric_fn])
        self.assertSameElements([ok_default_metric_fn], valid_task_2.metric_fns)
        self.assertEmpty(valid_task_2.score_metric_fns)
        self.assertSameElements([ok_default_metric_fn], valid_task_2.predict_metric_fns)
        ...
        valid_task_with_types = TaskRegistry.add(
            ...
            output_features={
                ...
                "targets": dataset_providers.Feature(test_utils.sentencepiece_vocab()),
            },
    ...
    def test_file_data_source_shuffle_buffer_low(self, mock_list_files):
        ...
        for _ in range(10):
            ds = [
                d.decode()
                for d in tfds.as_numpy(fds.get_dataset("train", shuffle=True, seed=23))
            ]
    ...
    def test_value_errors(self):
        dataset_fn = lambda split, shuffle_files: tf.data.Dataset.from_tensors(["test"])
        output_features = {
            "inputs": dataset_providers.Feature(test_utils.sentencepiece_vocab())
        }
    ...
    def test_no_eos(self):
        default_vocab = test_utils.sentencepiece_vocab()
        features = {
            "inputs": dataset_providers.Feature(add_eos=True, vocabulary=default_vocab),
            "targets": dataset_providers.Feature(
    ...
    def test_plaintext_to_pretokenized_rename(self):
        ...
        self.assertSetEqual(
            set(keys),
            set(["inputs", "inputs_pretokenized", "targets", "targets_pretokenized"]),
        )
...
class MixturesTest(test_utils.FakeTaskTest):
    ...
    def test_get_dataset_passthrough_features(self):
        ...
        self.add_task(
            "two_task",
            self.function_source,
            preprocessors=(functools.partial(_constant_feature_preprocessor, val=2),),
        )

        self.add_task(
            "three_task",
            self.function_source,
            preprocessors=(functools.partial(_constant_feature_preprocessor, val=3),),
        )
    ...
    def test_mixture_of_mixtures(self):
        ...
        self.assertEqual([supermix.get_rate(t) for t in supermix.tasks], [0.5, 0.5, 1])
    ...
    def test_mixture_of_mixtures_dupe(self):
        ...
        self.assertEqual([supermix.get_rate(t) for t in supermix.tasks], [1.5, 0.5, 1])
