...
from seqio import (dataset_providers, experimental, test_utils, utils,
                   vocabularies)
...
class FewshotTest(absltest.TestCase):
    def test_fewshot_data_source(self):
        def fake_dataset_fn(split, shuffle_files, seed=None):
            ...
            del shuffle_files
            del seed
            return tf.data.Dataset.range(*((0, 2) if split == "validation" else (3, 7)))
        ...
        # 1 shot
        preprocessors = [utils.map_over_dataset(lambda x: {"inputs": 0, "targets": x})]
        src = experimental.FewshotDataSource(
            dataset_providers.FunctionDataSource(
                dataset_fn=fake_dataset_fn, splits=["train", "validation"]
            ),
        ...
        # 3-shot, sharded.
        assert_dataset(
            src.get_dataset("validation", shuffle=False, shard_info=ShardInfo(0, 2)),
            [
                {"eval": 0, "train": {"inputs": [0, 0, 0], "targets": [3, 5, 5]}},
            ],
        )
        assert_dataset(
            src.get_dataset("validation", shuffle=False, shard_info=ShardInfo(1, 2)),
            [
    ...
    def test_fewshot_preprocessor(self):
        ...
        # 1-shot, batched eval
        dataset = experimental.fewshot_preprocessor(
            tf.data.Dataset.zip({"train": train_ds.batch(1), "eval": eval_ds.batch(2)}),
            inputs_prefix="0 ",
            targets_prefix=" X 1 ",
            example_separator=" X ",
        )
