...
from seqio import (dataset_providers, evaluation, feature_converters,
                   preprocessors)
...
_FAKE_DATASETS = {
    ...
    "token_preprocessed_ndfeatures": (_FAKE_TOKEN_PREPROCESSED_NDFEATURES_DATASET),
}
...
def get_fake_dataset(
    ...
    ds = tf.data.Dataset.from_generator(lambda: examples, output_types, output_shapes)
    if shard_info:
...
def assert_datasets_neq(dataset1, dataset2):
    ...
    _pyunit_proxy.assertRaises(AssertionError, assert_datasets_eq, dataset1, dataset2)
...
def split_tsv_preprocessor(dataset, field_names=("prefix", "suffix")):
    ...
    return dataset.map(parse_line, num_parallel_calls=tf.data.experimental.AUTOTUNE)
...
def test_postprocessing(
    ...
    with DataInjector(task_name, raw_data):
        evaluator = evaluation.Evaluator(task_name, feature_converter=feature_encoder)
...
def sentencepiece_vocab(
    extra_ids=0,
    normalizer_spec_overrides: Optional[sentencepiece_model_pb2.NormalizerSpec] = None,
...
class FakeTaskTest(absltest.TestCase):
    ...
    def _prepare_sources_and_tasks(self):
        ...
        _dump_fake_dataset(
            os.path.join(self.test_data_dir, "cached_plaintext_task", "train.tfrecord"),
