...
class TensorBoardLoggerTestV1(tf.test.TestCase):
    ...
    def test_logging(self):
        ...
        serialized_events = list(tfds.as_numpy(tf.data.TFRecordDataset(event_file)))[1:]
        event1 = tf.compat.v1.Event.FromString(serialized_events[0]).summary.value[0]
        rouge1 = event1.simple_value
        tag_rouge1 = event1.tag
        event2 = tf.compat.v1.Event.FromString(serialized_events[1]).summary.value[0]
...
class JSONLoggerTest(tf.test.TestCase):
    ...
    def test_logging_metrics_only(self):
        ...
        self.assertFalse(tf.io.gfile.exists(os.path.join(tmp_dir, "test-000042.jsonl")))
