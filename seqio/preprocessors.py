...
from seqio import feature_converters, utils
...
def _append_to_innermost_axis(tensor: tf.Tensor, scalar: tf.Tensor) -> tf.Tensor:
    ...
    if isinstance(tensor, tf.RaggedTensor):
        if tensor.shape.rank > 2:
            return tensor.with_values(_append_to_innermost_axis(tensor.values, scalar))
        else:
