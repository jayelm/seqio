...
from seqio import test_utils, utils
...
class MapOverDatasetTest(parameterized.TestCase):
    ...
    def test_random_map_fn_with_kwargs(self):
        ...
        for _ in range(3):  # reproducible with fixed initial seed
            with utils.map_seed_manager(initial_seed=123):
                mapped_ds = functools.partial(fn, val=1)(
                    ds
...
class UtilsTest(parameterized.TestCase, tf.test.TestCase):
    ...
    _tfexample = tf.train.Example(
        features=tf.train.Features(
            feature={
                "bool": tf.train.Feature(int64_list=tf.train.Int64List(value=[1, 0])),
                # NOTE: TFExamples only stores int64s, so we can't avoid the
                # up-casting here.
                "int32": tf.train.Feature(int64_list=tf.train.Int64List(value=[1])),
                ...
                "string": tf.train.Feature(bytes_list=tf.train.BytesList(value=[b"a"])),
    ...
    def test_trim_and_pad_dataset(self):
        ...
        assert_dataset(padded_ds, expected, {"inputs": tf.int32, "targets": tf.float32})
    ...
    def test_trim_and_pack_dataset(self, use_custom_ops):
        ...
        assert_dataset(packed_ds, expected, {"inputs": tf.int32, "targets": tf.int32})
    ...
    def test_autoregressive_inputs_packed(self):
        ...
        autoreg_inputs = utils.make_autoregressive_inputs(x, sequence_id=sequence_id)
        actual = self.evaluate(autoreg_inputs)
...
class MixtureRateTest(test_utils.FakeTaskTest):
    def test_mixing_rate_num_examples(self):
        ...
        self.assertEqual(
            9.0,
            utils.mixing_rate_num_examples(self.cached_task, scale=27, temperature=2),
        )
