...
def run_pipeline(
    ...
    for task_name in task_names:
        ...
        if not task.supports_caching:
            logging.info("Skipping task that does not support caching: '%s'", task.name)
            continue

        task_cache_dir = task.cache_dir
        output_dir = os.path.join(cache_dir, seqio.get_task_dir_from_name(task.name))
        ...
        for split in task.splits:
            ...
            completion_values.append(
                ...
                | "%s_write_stats" % label
                >> beam_utils.WriteJson(seqio.get_cached_stats_path(output_dir, split))
            )
        ...
        _ = (
            ...
            | "%s_discard_completion_values" % task.name >> beam.Filter(lambda _: False)
