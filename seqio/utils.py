...
TFDict = Dict[str, Union[np.ndarray, tf.Tensor, tf.RaggedTensor, tf.SparseTensor]]
...
def dict_to_tfexample(dct: TFDict, store_shapes: bool = False) -> tf.train.Example:
...
def unflatten_dict(dct: TFDict, delimiter=_TFEXAMPLE_NESTED_DELIMITER) -> NestedTFDict:
...
def _pack_with_tf_ops(
    ...
    dataset = dataset.map(pack_batch, num_parallel_calls=tf.data.experimental.AUTOTUNE)
...
def _pack_with_custom_ops(
    ...
    from tensor2tensor.data_generators.ops import \
        pack_sequences_ops  # pylint: disable=g-import-not-at-top
    ...
    for k in feature_lengths:
        ...
        if not element_spec[k].shape.is_compatible_with(tf.TensorShape([None, None])):
            use_generic_custom_ops = True
            logging.info(
                (
...
def make_autoregressive_inputs(
    ...
    if sequence_id is not None:
        not_first_in_sequence = tf.equal(sequence_id, _shift_right_by_one(sequence_id))
...
class _GrainRandomMapFn(_RandomMapTransform):
    ...
    def _map_fn_with_special_kwargs(self, *args, **kwargs):
        ...
        special_kwargs = {
            k: getattr(self, k) for k in _SPECIAL_KWARGS if getattr(self, k) is not None
        }
...
class _GrainMapFn(_MapTransform):
    ...
    # Path for SeqIO
    def __call__(self, dataset: tf.data.Dataset, *args, **kwargs) -> tf.data.Dataset:
        return dataset.map(
