...
def add_fully_cached_mixture(
    ...
    return MixtureRegistry.add(
        ...
        [
            (
                new_t.name,
                mixture._task_to_rate[old_t.name],
...
class FewshotDataSource(dataset_providers.DataSource):
    ...
    def __init__(
        ...
        eval_preprocessors: Iterable[Callable[[tf.data.Dataset], tf.data.Dataset]] = (),
...
def fewshot_preprocessor(
    ...
    def fewshot_map(ex):
        ...
        new_ex = {
            "inputs": (
                shots + inputs_prefix + ex["eval"]["inputs"] + targets_prefix.rstrip()
            ),
